...
The \verb\'\ is derived from the \emph{prime} notation, which indicates a derivative of the original~\cite{}.

For example when we have variable \verb\i\, this variable might get changed and is then indicated by \verb\i'\.
This shows that \verb\i'\ is derived from \verb\i\.

Instead of placing the \emph{prime} at the end, it is placed before the variable name to indicate it is a generic type.
