...
\verb\'a\ stands for \emph{alhpa}, \verb\'b\ for \emph{beta} and so on.
...
\section{Modules}
Modules have had two major changes, both will now be described.
...
Using these different syntaxes for .NET and MC creates a clear destinction between the two.
...
\textbf{IS IT REALLY CLEAR??????}
It is also immediately clear to the programmer a conditional is done.
The single \verb\=\ could be mistaken for an alias creation or value assignment.
