...
The parameters are seperated by the ascii arrow, \verb|->|.
...
The conclusion is placed underneath the bar and the premises above.

Now lets define a rule for \verb\computeNumber\.
...
This syntax is similiar to that of natural deduction~\cite{prawitz1965natural}.

\subsubsection{Locally defined identifiers}
The identifiers named in the conclusion of a function are locally defined.
With the function \verb\computeNumber\ the identifiers \verb\a\, \verb\b\ and \verb\c\ are examples of these local identifiers.
...
If the a rule fails, the rule beneath is activated.
If none of the rules match, then the program crashes.
...
Other conditionals have to be put in the premises, because they need to be computed.\footnote{See section~\ref{sec:basicfunc}}
...
Because the \verb\|\ (\emph{pipe}) operator\footnote{As used in section~\ref{sec:basicdata}} is not built in the language we need to create it.
To do this we need to manipulate types.

To make \verb\|\ work, we both need a constructor and deconstructor that works with kinds.
...
There is a limitation to the use left arguments, namely there may only be one.
...
Because the benefits of having multiple arguments in front of the function name does not outweigh the time it consumes to expand the parser, the choice was made to only allow one argument in front.
...
When we want to call a function within a module from outside the module, the \verb\^\ operator is used.

\begin{lstlisting}
Func "caretTest" -> Int
caretTest -> identityAdd^MonoidAdd
\end{lstlisting}

This creates a constant named \verb\caretTest\ with the same value as the \verb\identityAdd\ from the module \verb\MonoidAdd\.
Important to note is that we cannot call a function outside the module from within the module.
The \verb\^\ notation is derived from \LaTeX.
In \LaTeX\space the \verb\^\ is used to indicate superscript.
This is used is MC to show that one element is part of another element like so: \verb~identityAdd~\textsuperscript{\texttt{MonoidAdd}}.
...
\verb\#>\ is also used to indicate associativity.
...
When the programmer wants to explicitly specify to use a function from the imported file, then \verb|^| is used.

For example we can import the file ``vector'' and use the \verb\Vector2\ type from it, as seen below:
...
\subsection{Builtin}
Some things cannot be created out of nothing and need to be builtin in the compiler.
An example of this is the boolean data type.

When using such a builtin literal the keyword \verb\builtin\ is used.
We can now correctly implement the function \verb\foo\\footnote{As used in section~\ref{sec:basicfunc}} using the \verb\builtin\ keyword:
...
\subsection{Lambdas}
MC also has anonymous functions to implement lambdas.
...
\verb\ArrowFunc\ always needs an argument on the left of the name and at least one function to the right.
A unique feature of \verb\ArrowFunc\ is the two syntax options it has when the function gets called.

The first is a regular function and the second converts it to a lambda.
