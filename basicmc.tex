...
As we will see in section~\ref{sec:basictypefunc}, MC also has the ability to declare and define functions that run on compile time.
...
This is can be seen when we declare the following function:
...
We can use this syntax in declarations to indicate a function is used as the argument.


\subsection{Data}\label{sec:basicdata}
With \verb|Data| we can declare a constructor and deconstructors for a new type.
Because it can work two ways it effectively creates an alias.
...
Other conditionals have to be put in the premises, because they need to be computed.\footnote{As can be seen in section~\ref{sec:basicfunc}}
...
To indicate a function is resolved or inlined on compile time, the \verb\=>\ arrow is used.
...
We have already seen that \verb\TypeFunc\ can do this.\footnote{See section~\ref{sec:basictypefunc}}
...
\subsection{Module}\label{sec:basicmodule}
...
Say we want to create a container for every type which does an addition of two identifiers of those types.
...
The priority indicitates the order of function execution.
