...
The complete code samples can be found in~\cite{}.

Each item will be explained with the evolution after the explanation.
...
\subsubsection{List}
\textbf{DO THIS!!!!!!!!!!}
list was first put in the list monad but moved to prelude, because it is a generic list function not specific to the list monad.
...
With the original version of \verb\set\ and \verb\get\ the field-argument was checked to be the same type offor the same type of the current value of the field
