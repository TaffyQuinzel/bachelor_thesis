...
They contain atleast two functions, \emph{return} and \emph{bind}.
...
The \verb\>>=\ (pronounced \emph{bind}) function supplies this functionality.

When we want to apply the function \verb\(+3)\ to the monad containing \verb\2\, we call the bind.
The bind unpacks the monad, as visualised in figure~\ref{fig:monadbind1}.
...
And the the new value gets packed inside the monad by the function, as visualised in figure~\ref{fig:monadbind2}.
...
\subsubsection{Lift}
Lift is used when a regular function, it takes a value and returns a value, needs to be executed over a monad.
...
\subsubsection{LiftM}
There are of course functions which work with the wrapped monad.
...
The monad can now be used with a wider aspect within programming.
...
This means a lot of different functionalities can be mapped to monads.
...
The implementations of these monads are far from complete, these examples simply show the functionality of the monad.
The actual implementation will be given in section~\ref{sec:basicmonadsimplementation}.


\subsubsection{Combining monads}
Monads can also be combined.
Since they have the same generic interface they can be combined into another generic inteface.
These new monads can utilise the functionality of the all combined monads.

In this way the functionality of a monad can be extended.
...
% {
   ...
   % \captionof{figure}{Combining monads}
   % \label{fig:monadcombining}
   % \bigskip
% }

We will take the state and maybe monad and combine these into a parser monad.
A parser monad can parse through a piece of text and scan for certain elements.
...
Instead of simply having a monad which takes the arguments it needs, it also takes another monad transformer as one of its arguments.
This monad transformer is then used to given the actual value.
...
Because monad transformers need to be able to manipulate type information, it is a good example of what MC is useful for.

With this monad transformer we will be able to create actual monads, this can be seen as the base module for monads.
...
The functions explained and implemented in section~\ref{} will be added to this aswell.

With those we have the complete interface of the monad transformer.
With this we can combine and built monads automatically.

\subsection{Evolution of monad}
The monad was first implemented with just the \verb\MCons\, \verb\>>=\, \verb\return\ and \verb\returnFrom\ functions.
This enabled basic use of the monad.

The \verb\lift\, \verb\lift2\ and \verb\liftM\ were then added to extend the usecases of monads.
...
The result of the \verb\match\ is then wrapped in monad \verb\'M\ and returned.
...
Now we can create a \verb\Monad\ which has two options when calling the bind.
This can be used for error handling or for creating complex dataflows within monads.


\subsection{Evolution of tryable monad}
\textbf{DO THIS !!!!!!!!!!!!!!!!!!!}
The \verb\TryableMonad\ startet out as the \verb\either\ monad.
The \verb\either\ used the available options to create error handling.
...
If monad transformers always take another monad, there has to be atleast one to end the chain.
That is where the \emph{id} monad transformer comes in.
...
The type signature is created with \verb\Id\ and the bind and return are defined.
...
As an example we pass the \verb\id\ monad to the maybe monad transformer.
When the return of the created maybe monad is called, it first calls the return of the inner monad.
In this case is the \verb\id\ monad.

The return of the \verb\id\ monad returns the value directly as a result.
This result is then passed back to the maybe monad, where it gets wrapped inside the maybe monad and returned.
...
The list monad transformer uses \verb\List\ from \emph{prelude} to create the transformer.
...
As stated in section~\ref{sec:basicmonadsimplementedid} the programmer has to decide which type the list monad uses.
...
The list that the is used within the list monad, was first implemented within the list monad.
...
The either monad is an implementation of the \verb\TryableMonad\ module.

It creates a monad which can be either a value or a list of error messages.
...
It takes an error message and concatenates it with the existing error messsages.
...
\verb\Left\ is used because \verb\x\ is a value.

\begin{lstlisting}
  return x -> return^'M(Left x)
\end{lstlisting}

\textbf{maybe put a sort of conclusion here}
The \verb\either\ can now be


\subsubsection{Evolution of either}
The \verb\either\ was first implemented without the use of \verb\TyableMonad\.
It had the same functionality.
All the functions of the \verb\TryableMonad\ were implemented directly within \verb\either\.

When the idea occured of using a generic tryable monad, these were ported to the \verb\TryableMonad\.
The \verb\either\ was then implemented by using the \verb\TryableMonad\.


% \subsection{Option}
% \textbf{explain that it is obsolete because of either}
...
% }
% \end{lstlisting}
% \subsubsection{Evolution of option}
...
In the declaration we specify it returns a \verb\TryableMonad\, which \verb\either\ is.
And in the definition we specify the error type to be \verb\String\.
...
The original idea of the \verb\result\ monad was a maybe monad with error handling in the form of a \verb\string\.
...
\subsection{State}
We have had a basic explanation of the state monad in section~\ref{sec:standardmonad}.

Now we will see how it is implemented.
The type signature of the state monad shows that it is a function.
...
We know \verb\x\ is a tuple because it lives inside a state monad.
...
Haskell has a few implementations of monad transformers~\cite{monadshaskell}, but these are far longer than the MC implemntation.\footnote{See appendix~\ref{app:monadshaskell} for the source code of a few of these monadic transformers}
...
% \subsubsection{Evolution of state}
% \subsection{IO}
