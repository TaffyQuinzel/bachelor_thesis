...
\textbf{EXPLAIN WHY MONADS ARE IMPLEMENTED}
...
\textbf{PUT THIS AFTER MONAD IMPLEMENTATION}
...
Like normal monads, transformers have to be built manually.
...
\textbf{PUT IMPLEMENTATION OF FUNCTIONS HERE}
...
Now we will look at how the monad is implemented.
...
The functions explained and implemented in section~\ref{something} will be added to this as well.

With those we have the complete interface of monad.
With this we can build monads.
...
That is where the \emph{id} monad comes in.
...
The \verb\list\ monad uses a list to store the values.
...
\textbf{PUT A SORT OF CONCLUSION HERE}
...
\subsection{Result}
The result monad is a complete implementation of the either monad from section~\ref{sec:basicmonadseither}.
...
\subsection{State}
\textbf{EXPLAIN THAT BOX IS NOT A CONTAINER THIS TIME}
